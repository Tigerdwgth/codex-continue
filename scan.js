...
/** 同步 sleep（Atomics.wait 不占事件循环之外的资源）。 */
function sleep(ms) {
 ...
 * 往 pane 发送指定文本 + 回车。
 *
 * 必须**分两次** send-keys、中间留一点延迟，不能一次 `send-keys 'continue' Enter`：
 * codex 的 TUI 对 tmux 连续按键流会吞掉紧跟文本的 Enter —— 实测一次发送时文本进了
 * 输入框但回车不提交（输入框挂着 continue），而先发文本、稍后再单独发 Enter 才正常
 * 提交（codex 进入 Working）。普通 shell / 其它程序无此问题，属 codex 输入处理时序。
 */
function sendKeys(exec, target, send, enterDelayMs = 200) {
  exec(['send-keys', '-t', target, send]);
  if (enterDelayMs > 0) sleep(enterDelayMs);
  exec(['send-keys', '-t', target, 'Enter']);
}
 ...
 * @param {number} [opts.enterDelayMs] 文本与回车之间的延迟，默认 200（codex 吞键规避）
 * @param {() => number} [opts.now] 时钟（毫秒）
 * @param {(msg: string) => void} [opts.onTrigger] 每次触发后的回调（日志用）
 * @returns {{ scan: () => number, listCodexPanes: () => string[] }}
 */
function createScanner({ exec, matchers, cooldownMs, send, enterDelayMs, now, onTrigger } = {}) {
  ...
  function scan() {
    ...
    for (const target of targets) {
      ...
      state.lastAt = t;
      state.lastSig = sig;
      sendKeys(run, target, text, delay);
      triggered += 1;
