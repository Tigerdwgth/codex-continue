...
/** 检测日志尾部（从 fromBytes 到末尾）是否有容量/429 错误。返回 { hit, newSize }。 */
function detectError(logPath, fromBytes, patterns, execRead = fs.readFileSync, execStat = fs.statSync) {
  let size = 0;
  try { size = execStat(logPath).size; } catch { return { hit: false, newSize: 0 }; }
  if (size <= fromBytes) return { hit: false, newSize: size };
  const start = Math.max(0, fromBytes, size - LOG_TAIL_BYTES);
  let chunk = '';
  try { chunk = execRead(logPath, { encoding: 'utf8', start, end: size }); } catch { return { hit: false, newSize: size }; }
  return { hit: patterns.some((re) => re.test(chunk)), newSize: size };
}
...
function createScanner({
  execPs, matchers, cooldownMs, send, enterDelayMs, now, onTrigger, sessionsDir, io,
} = {}) {
  ...
  function scan() {
    ...
    for (const proc of procs) {
      ...
      if (!st) {
        st = { logPath: log.path, lastSize: 0, lastAt: Number.NEGATIVE_INFINITY };
      ...
      const { hit, newSize } = detectError(log.path, st.lastSize, patterns, ioImpl.readFileSync, ioImpl.statSync);
      st.lastSize = newSize;
      if (!hit) continue;
