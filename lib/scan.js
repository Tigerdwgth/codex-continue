...
function createScanner({
  ...
  function scan() {
    ...
    for (const proc of procs) {
      ...
      //    + 429 是最近发生的（3 分钟内）——历史残留（如超长会话里早前的 429）
      //    不能当"现在卡住"，否则每轮扫描撞上写入间隙就反复误发 continue
      // 2. tmux 屏幕实时显示 429/capacity 错误
      //
      // 关键：屏幕命中 ≠ 真卡住。codex 收到 continue 恢复工作后，TUI 上的 429
      // 错误行可能残留显示，capture-pane 仍匹配到 —— 但日志在写就说明 codex 活着。
      // 所以 screenHit 命中时也要确认日志**没在活跃增长**（mtime 陈旧 = 真卡住），
      // 日志在写（mtime 新鲜）则视为屏幕残留，不打扰。
      const logActive = mtimeMs !== 0 && t - mtimeMs < STALE_MS;
      if (screenHit) {
        if (logActive) continue; // 屏幕 429 是残留，codex 在写日志干活
      } else {
        if (!hit) continue;
        const logStale = !logActive; // 日志停滞 = codex 没在写
        // 日志 mtime 新鲜（在写）= codex 活跃/正在自动重试，不打扰
        if (!logStale) continue;
        // 日志已停滞但 429 后还有活动（如正常完成任务、日志最后不是错误）→ 不推
        if (activeAfterError) continue;
        // 429 是历史残留（超过 3 分钟前）→ 不推，避免对已恢复会话反复发
...
module.exports = {
  ...
  VERIFY_WINDOW_MS, SILENCE_MS, STALE_MS, ERROR_RECENT_MS,
};
