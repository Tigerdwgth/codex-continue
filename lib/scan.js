...
const MATCHERS = [
  ...
  // 403 访问拒绝（网关返回）也发 continue。\b 保证只匹配独立 403 数字，
  // 不误伤 base64（x403... 前是字母无词边界）和路径（-retry2 无独立 403）。
  /\b403\b/i,
  /forbidden/i,
  /access denied/i,
  // 404 endpoint 不存在 / 网关路由错误（unexpected status 404 Not Found）。
  // 独立 404 数字可能是 base64/路径，用更具体的 SDK 错误格式优先。
  /unexpected status 404 not found/i,
  /\b404 not found\b/i,
];
...
const SILENCE_MS = 10 * 60 * 1000;      // 发送后的冷静期，期间不再发，避免反复推同一次卡住
...
// 日志检测只认「最近发生的 429」：超过这个时间的 429 是历史残留（超长会话里早前
// 卡过、已恢复），不能当"现在卡住"。避免每轮扫描撞上日志写入间隙就反复误发。
const ERROR_RECENT_MS = 3 * 60 * 1000;
// 屏幕 429 残留需要日志停滞超过这么久才当真：codex 恢复干活后屏幕错误行会被新内容
// 刷掉，短暂停留不算卡住。只有日志彻底不写（停滞 >90s）且屏幕仍有 429 才是真卡死。
const SCREEN_STALE_MS = 90 * 1000;
 ...
 * - 普通终端：直接写 tty 设备，回车用 `\n`
 */
function sendContinue(proc, send, enterDelayMs, { sendTmux, writeTty, onTrigger } = {}) {
  const run = sendTmux || defaultExecTmux;
  const write = writeTty || fs.writeFileSync;
  const notify = onTrigger || (() => {});
  if (proc.tmuxTarget) {
    run(['send-keys', '-t', proc.tmuxTarget, send]);
    if (enterDelayMs > 0) sleep(enterDelayMs);
    // Enter 提交。codex 输入框刚出现/处于错误处理态时，首个 Enter 偶尔被当成
    // 换行（多行输入模式）而不是提交 → continue 文本残留输入框没发送。
    // 所以发完 Enter 后再看一次输入框：若 continue 文本还在（没提交），补发一次
    // Enter 确保提交。
    run(['send-keys', '-t', proc.tmuxTarget, 'Enter']);
    if (enterDelayMs > 0) sleep(enterDelayMs);
    const paneOut = run(['capture-pane', '-p', '-t', proc.tmuxTarget]);
    // 输入框提示符（» / >）后还跟着刚发的文本 = Enter 被当成换行没提交
    if (paneOut && new RegExp(`[»›>]\\s*${escapeRegex(send)}`, 'i').test(paneOut)) {
      notify(`首 Enter 未提交（${proc.tty}），补发 Enter`);
      run(['send-keys', '-t', proc.tmuxTarget, 'Enter']);
    }
  } else {
 ...
 * @param {number} [opts.silenceMs] 发送后的冷静期，期间不再发，默认 10 分钟
...
function createScanner({
  execPs, execTmux, execLsof, matchers, cooldownMs, send, enterDelayMs, silence, now, onTrigger, sessionsDir, io,
} = {}) {
  ...
  function scan() {
    ...
    for (const proc of procs) {
      ...
      if (!st) {
        st = { logPath: log.path, lastSize: 0, lastAt: Number.NEGATIVE_INFINITY, silencedUntil: 0 };
      ...
      // 发送后进入强制冷静期（silencedUntil），期间无条件不再发。理由：
      // codex 收到 continue 恢复后可能很快又撞 429（长任务高频限流），若每次 detect
      // 到就立刻重发，会形成「continue→又 429→再 continue」的死循环并堆积输入。
      // 只有冷静期过后，日志**新增**了 429 且真停滞，才需要再发一次。
      if (t < st.silencedUntil) continue;
      ...
      st.lastSize = newSize;
      const logActive = mtimeMs !== 0 && t - mtimeMs < STALE_MS; // 日志在写 = codex 活跃

      // 屏幕检测：tmux 会话看屏幕文本。只当「日志停滞 >SCREEN_STALE_MS 且屏幕仍有
      // 429」才采信 —— codex 某些 429 只显示在 TUI 上不写日志，此时日志长时间不涨
      // 就是真卡死。日志在写（哪怕是处理 continue）则屏幕 429 是残留，不触发。
      let screenHit = false;
      if (proc.tmuxTarget && !logActive) {
        const staleFor = mtimeMs !== 0 ? t - mtimeMs : STALE_MS;
        if (staleFor >= SCREEN_STALE_MS) {
          try {
      ...
      // 触发 = 日志检测到「新的 429」（st.lastSize 增量窗口内）且日志停滞
      // 或 屏幕 429 且日志已停滞超过 90s（codex 不写日志的真卡死兜底）。
      if (!screenHit) {
        if (!hit) continue;
        // 日志 mtime 新鲜（在写）= codex 活跃/正在自动重试，不打扰
        if (logActive) continue;
        // 日志停滞但 429 后还有活动（正常完成任务、日志最后不是错误）→ 不推
        if (activeAfterError) continue;
        // 429 是历史残留（超过 3 分钟前）→ 不推
        if (lastErrorTime !== null && t - lastErrorTime > ERROR_RECENT_MS) continue;
      ...
      try {
        sendContinue(proc, text, delay, { sendTmux: runTmux, writeTty: ioImpl.writeFileSync, onTrigger });
        triggered += 1;
        // 无条件进入冷静期：无论 continue 是否立即使会话恢复，冷静期内都不再发。
        // 给 codex 足够时间自行处理后续 429，避免同一次卡住被反复推。
        st.silencedUntil = t + silenceMs;
        if (onTrigger) onTrigger(`sent "${text}" to ${proc.tty} (${proc.pid})${proc.tmuxTarget ? ` via tmux ${proc.tmuxTarget}` : ''}，冷静 ${Math.round(silenceMs / 60000)} 分钟`);
      } catch (e) {
...
module.exports = {
  ...
  SILENCE_MS, STALE_MS, ERROR_RECENT_MS, SCREEN_STALE_MS,
};
