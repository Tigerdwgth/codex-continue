 ...
 * 时间戳。只统计能解析出 timestamp 的错误行 —— 无法解析的（如截断行）不计，
 * 避免把历史错误或损坏行当新错误误触发。
...
function detectError(logPath, fromBytes, patterns, execRead = fs.readFileSync, execStat = fs.statSync) {
  ...
  for (const line of chunk.split('\n')) {
    ...
    let ts = null;
    try { ts = Date.parse(JSON.parse(line).timestamp); } catch { /* 行不完整或非 JSON，跳过 */ }
...
function createScanner({
  execPs, matchers, cooldownMs, send, enterDelayMs, errorWindowMs, now, onTrigger, sessionsDir, io,
} = {}) {
