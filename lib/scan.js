...
function findSessionLog(proc, allLogs, assigned, openLogs) {
  ...
  // lsof 探测优先：codex 进程真实打开的文件。选未分配的、最近写入者。
  // （真正「卡住要触发」的会话由 scan() 里对每个 openLog 单独检测来定位，
  //   这里只负责给出一个候选，供无 openLog 时的回退和状态缓存使用。）
...
function detectError(logPath, fromBytes, patterns, execRead = fs.readFileSync, execStat = fs.statSync) {
  let size = 0;
  try { size = execStat(logPath).size; } catch { return { hit: false, newSize: 0, lastErrorTime: null, activeAfterError: false }; }
  if (size <= fromBytes) return { hit: false, newSize: size, lastErrorTime: null, activeAfterError: false };
  const start = Math.max(0, fromBytes, size - LOG_TAIL_BYTES);
  let chunk = '';
  try { chunk = execRead(logPath, { encoding: 'utf8', start, end: size }); } catch { return { hit: false, newSize: size, lastErrorTime: null, activeAfterError: false }; }
...
function createScanner({
  ...
  function scan() {
    ...
    for (const proc of procs) {
      ...
      // 从进程真实打开的会话日志里，选「有 429 错误且错误后无新活动」的那个。
      // 一个 codex resume 进程可能打开多个历史会话，只有真正停在 429 上的需要推。
      const candidates = openLogs.length > 0
        ? openLogs.map((p) => allLogs.find((l) => l.path === p)).filter(Boolean)
        : [];
      let log = null;
      let candHit = false;
      for (const c of candidates) {
        const { hit, lastErrorTime, activeAfterError } = detectError(c.path, 0, patterns, ioImpl.readFileSync, ioImpl.statSync);
        if (!hit || activeAfterError) continue;
      ...
      if (!log) {
        log = findSessionLog(proc, allLogs, assigned, candidates);
        if (!log) continue;
      }
      ...
      const { hit, newSize, lastErrorTime, activeAfterError } = detectError(log.path, st.lastSize, patterns, ioImpl.readFileSync, ioImpl.statSync);
      st.lastSize = newSize;
      if (!hit) continue;

      // 错误之后会话已有新活动（已恢复 / 用户在继续 / codex 自己重试成功）→ 不需要 continue
      if (activeAfterError) continue;
      // 进程在最近一次错误之后才启动 = 用户 resume / 重开了这个会话（如刚 resume 历史
      // 会话，日志里带着之前的 429），此时由用户自己控制，不能自动 continue 打扰。
      if (proc.startTimeMs !== null && lastErrorTime !== null && proc.startTimeMs > lastErrorTime) continue;
