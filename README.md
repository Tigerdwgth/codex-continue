...
3. 触发检测（双信号）：
   - **tmux 屏幕实时检测**：对 tmux 里的会话用 `tmux capture-pane` 看屏幕文本，
     实时显示 429 / capacity / **403 访问拒绝**错误就触发 —— codex 的某些错误只
     显示在 TUI 上、不写日志，只能靠屏幕判断「现在是否卡住」；
   - **日志检测**：日志**新增长**部分出现 429 / 容量 / 403 错误（不区分大小写，
     滚动历史不重复触发），且「错误是最后一条、日志已停滞（codex 没在写）」才触发；
   - 防误发：日志仍在写（codex 活跃/自动重试中）、错误后已有新活动（已恢复）、
     错误是 3 分钟前的历史残留、进程刚 resume 历史会话、或冷却期内，一律不打扰；
